...
public class LoginController {
	...
	public String[] Oses() throws Exception{
		MemberRegistRequest memberRegistRequest = new MemberRegistRequest();
		String Oses = "윈도우XP,윈도우7";
		memberRegistRequest.setFavoriteOs(Oses.split(","));
		return memberRegistRequest.getFavoriteOs();
...
/*
	...
	public String login(HttpServletResponse response,@ModelAttribute("loginCommand") LoginCommand loginCommand,
		...
		System.out.printf("LoginController POST.build() 실행 \n");
		...
		LOGGER.debug("#### Success Save User : UserName is  {}, Email is {} " ,loginCommand.getEmail(),loginCommand.getPassword());
		
		new AuthValidator().validate(loginCommand, bindingResult);
...
*/
	/*
	@RequestMapping(method = RequestMethod.POST)
	public String login(HttpServletResponse response,@Valid LoginCommand loginCommand,
	...
	*/
	...
	public String login(HttpServletResponse response,@Valid @ModelAttribute("loginCommand") LoginCommand loginCommand,
		...
		try {
			/*
			Auth auth = new Auth(loginCommand.getEmail(),loginCommand.getPassword());
	...
	protected void initBinder(WebDataBinder binder){
		...
		CustomDateEditor dateEditor = new CustomDateEditor(new SimpleDateFormat("HH:mm"),false);
